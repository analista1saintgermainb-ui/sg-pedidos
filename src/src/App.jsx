...
async function dbLoad() {
  const r = await fetch(`${SUPA_URL}/rest/v1/pedidos?select=*&order=id&limit=10000`, { headers: SH })
...
async function dbDelete(id) {
  await fetch(`${SUPA_URL}/rest/v1/pedidos?id=eq.${id}`, { method: "DELETE", headers: SH })
}
async function dbClear() {
  await fetch(`${SUPA_URL}/rest/v1/pedidos?id=gte.0`, { method: "DELETE", headers: SH })
}
...
function getTemplate(r, ch) {
  ...
  if (ch==="wpp") {
    if (extrav) return `Olá, ${nome}! 😊\n\nAqui é a equipe *Saint Germain*. Entramos em contato sobre o pedido *#${r.nuvem}*.\n\nSua encomenda está com status de *objeto extraviado* junto à *${r.transportadora}*. Já acionamos nossa equipe.\n\nRetornaremos em até *2 dias úteis*. Pedimos desculpas! 🙏`
    if (devolv) return `Olá, ${nome}! 😊\n\nAqui é a equipe *Saint Germain*. Passando sobre o pedido *#${r.nuvem}*.\n\nSua encomenda foi *devolvida* ao nosso CD. Poderia confirmar o endereço para novo envio sem custo? 📦`
    if (atraso) return `Olá, ${nome}! 😊\n\nAqui é a equipe *Saint Germain*. Sobre o pedido *#${r.nuvem}* — identificamos um atraso pela *${r.transportadora}*. Estamos acompanhando! 🙏`
    return `Olá, ${nome}! 😊\n\nAqui é a equipe *Saint Germain*. Atualizando sobre o pedido *#${r.nuvem}*.\n\nStatus: *${r.status}*${r.prazo?`\nPrazo: *${r.prazo}*`:""}\n\nQualquer dúvida é só chamar! ✨`
  } else {
    if (extrav) return `Assunto: Pedido #${r.nuvem} — Objeto Extraviado\n\nOlá, ${r.destinatario},\n\n${det}\nStatus: Objeto extraviado\n\nNossa equipe está apurando com a transportadora e retornamos em até 2 dias úteis.\n\nAtenciosamente,\nEquipe Saint Germain`
...
function SlaCell({ prazo }) {
  ...
  return (
    <div style={{lineHeight:1.5}}>
      <div style={{fontSize:11,color:"#888"}}>{prazo||"—"}</div>
      {sla && <span style={{background:sla.bg,color:sla.color,borderRadius:2,padding:"1px 5px",fontSize:10,fontWeight:500}}>{sla.label}</span>}
    </div>
  )
}
...
function StatCard({ label, val, accent, sub }) {
  return (
    <div style={{background:"#fff",borderRadius:6,padding:"14px 16px",border:accent?"1px solid #f5c6cb":"1px solid #e8e8e8"}}>
      <div style={{fontSize:10,color:"#999",marginBottom:4,letterSpacing:"0.05em",textTransform:"uppercase"}}>{label}</div>
      <div style={{fontSize:26,fontWeight:500,color:accent?"#c0392b":GOLD,letterSpacing:"-0.02em"}}>{val}</div>
...
function CopyBtn({ text, label }) {
  ...
  return (
    <button onClick={()=>{navigator.clipboard.writeText(text);setOk(true);setTimeout(()=>setOk(false),2000)}}
      style={{background:ok?"#d4edda":GOLD,border:"none",color:ok?"#155724":"#111",borderRadius:3,padding:"4px 12px",fontSize:11,cursor:"pointer",fontWeight:500,whiteSpace:"nowrap"}}>
...
function Toast({ toasts }) {
  return (
    <div style={{position:"fixed",bottom:20,right:20,display:"flex",flexDirection:"column",gap:8,zIndex:9999,pointerEvents:"none"}}>
      {toasts.map(t => (
        <div key={t.id} style={{background:t.type==="error"?"#c0392b":t.type==="warn"?GOLD:"#27ae60",color:"#fff",borderRadius:4,padding:"10px 16px",fontSize:12,fontWeight:500,boxShadow:"0 4px 12px rgba(0,0,0,0.15)",maxWidth:300,lineHeight:1.4}}>
          {t.msg}
        </div>
      ))}
    </div>
...
export default function App() {
  ...
  const [tab, setTab]             = useState("logistica")
  ...
  useEffect(() => {
    const onKey = e => {
      ...
      if (tab!=="suporte"||!selSup) return
      const detail = rows.find(r => r.id===selSup); if (!detail) return
      if (e.key==="Escape") setSelSup(null)
      if (e.key==="Enter"&&detail.atendimento==="Aberto") { e.preventDefault(); handleInitiate(selSup) }
      if ((e.key==="r"||e.key==="R")&&detail.atendimento==="Em andamento") handleResolve(selSup)
    }
  ...
  const doImport = useCallback(txt => {
    ...
    let added = 0
    setRows(prev => {
      const ex = new Set(prev.map(r => r.nuvem+"|"+r.nf))
      const novos = parsed.filter(r => !ex.has(r.nuvem+"|"+r.nf))
      added = novos.length
      return [...prev, ...novos]
    })
    setTimeout(() => {
      if (added>0) addToast(`${added} pedido${added>1?"s":""} importado${added>1?"s":""}!`)
      else addToast("Nenhum pedido novo — todos já existem.", "warn")
    }, 100)
  ...
  const handleFile = e => {
    ...
    if (/\.(xlsx?)$/i.test(f.name)) {
      ...
      rd.onload = ev => { const wb=XLSX.read(new Uint8Array(ev.target.result),{type:"array"}); doImport(XLSX.utils.sheet_to_csv(wb.Sheets[wb.SheetNames[0]],{FS:";",blankrows:false})) }
      rd.readAsArrayBuffer(f)
    } else {
      const enc = /\.csv$/i.test(f.name)?"windows-1252":"UTF-8"
      const rd = new FileReader(); rd.onload = ev => doImport(ev.target.result); rd.readAsText(f, enc)
    }
    e.target.value = ""
  }
  ...
  const del = id => { setRows(prev => prev.filter(r => r.id!==id)); dbDelete(id).catch(()=>{}) }
  ...
  const handleInitiate = id => { upd(id,{atendimento:"Em andamento"},{acao:"Atendimento iniciado"}); setSelSup(id) }
  const handleResolve  = id => { upd(id,{atendimento:"Resolvido"},{acao:"Status → Resolvido"}); setSelSup(null); addToast("Pedido resolvido e arquivado") }
  const handleReturnLog = id => { upd(id,{enviadoSuporte:false,sentAt:null},{acao:"Devolvido à Logística"}); setSelSup(null) }
  const handleClearAll = () => {
    if (!confirmClear) { setConfirmClear(true); setTimeout(()=>setConfirmClear(false),4000); return }
    setRows([]); setConfirmClear(false); dbClear().catch(()=>{})
  ...
  const supRows = baseSup
    .filter(r=>{const q=sSrch.toLowerCase();return(!q||[r.nuvem,r.destinatario,r.rastreio,r.nf,r.status].some(v=>v.toLowerCase().includes(q)))&&(sAtend==="Todos"||r.atendimento===sAtend)&&(sUrg==="Todos"||r.urgencia===sUrg)})
  ...
  const archRows = baseArch
    .filter(r=>{const q=aSrch.toLowerCase();return!q||[r.nuvem,r.destinatario,r.transportadora,r.status,r.nf].some(v=>v.toLowerCase().includes(q))})
    .sort((a,b)=>{const ta=(a.historico.find(h=>h.acao&&(h.acao.includes("Resolvido")||h.acao.includes("Arquivado")))||{}).ts||"";const tb=(b.historico.find(h=>h.acao&&(h.acao.includes("Resolvido")||h.acao.includes("Arquivado")))||{}).ts||"";return tb.localeCompare(ta)})

  const stOpts = uniq(baseLog.map(r=>r.status))
  const trOpts = uniq(baseLog.map(r=>r.transportadora))
  ...
  const slaData = [
    {name:"No prazo", value:rows.filter(r=>{const d=parsePrazo(r.prazo);return d&&d>=hoje}).length, fill:"#27ae60"},
    {name:"Entregues",value:rows.filter(r=>(r.status||"").toLowerCase().includes("entregue")).length, fill:GOLD},
    {name:"Vencidos", value:rows.filter(r=>{const d=parsePrazo(r.prazo);return d&&d<hoje&&!(r.status||"").toLowerCase().includes("entregue")}).length, fill:"#e74c3c"},
  ].filter(d=>d.value>0)

  const showImp = importing||rows.length===0
  const pd  = compact?4:7
  const ct  = "#999"
  ...
  const TABS = [{key:"dashboard",label:"Dashboard",badge:null},{key:"logistica",label:"Logística",badge:st.acionar>0?st.acionar:null},{key:"suporte",label:"Suporte",badge:ss.abertos>0?ss.abertos:null},{key:"arquivados",label:"Arquivados",badge:arch>0?arch:null}]
  const changeTab = k => { setTab(k); if(k!=="suporte") setSelSup(null) }
  const syncColor = syncStatus==="error"?"#e74c3c":syncStatus==="saving"?GOLD:syncStatus==="saved"?"#27ae60":syncStatus==="loading"?"#888":"#555"
  const syncDot   = syncStatus==="error"?"#e74c3c":syncStatus==="saving"?GOLD:syncStatus==="saved"?"#27ae60":"#444"
    ...
    <div style={{fontFamily:"'Helvetica Neue',Arial,sans-serif",minHeight:"100vh",background:"#fff"}}>
      ...
      <div style={{background:BRAND,padding:"13px 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          {!showImp&&rows.length>0&&<button onClick={()=>setCompact(c=>!c)} style={{background:"transparent",border:"1px solid #333",color:compact?GOLD:"#555",borderRadius:2,padding:"4px 10px",fontSize:10,cursor:"pointer"}}>{compact?"⊞ Normal":"⊟ Compacto"}</button>}
          {!showImp&&rows.length>0&&(
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{const hf=lSrch||lSt!=="Todos"||lTr!=="Todos"||lUrg!=="Todos"||lAc!=="Todos";exportCSV(hf&&tab==="logistica"?filteredLog:rows)}} style={{background:"transparent",border:"1px solid #444",color:"#aaa",borderRadius:2,padding:"5px 12px",fontSize:11,cursor:"pointer"}}>↓ Exportar</button>
              <button onClick={()=>setImporting(true)} style={{background:"transparent",border:`1px solid ${GOLD}`,color:GOLD,borderRadius:2,padding:"5px 12px",fontSize:11,cursor:"pointer"}}>+ Importar</button>
              <button onClick={handleClearAll} style={{background:confirmClear?"#c0392b":"transparent",border:`1px solid ${confirmClear?"#c0392b":"#444"}`,color:confirmClear?"#fff":"#666",borderRadius:2,padding:"5px 12px",fontSize:11,cursor:"pointer"}}>{confirmClear?"⚠ Confirmar":"Limpar dados"}</button>
            </div>
          )}
        </div>
      ...
      {!showImp&&(
        <div style={{background:"#fff",borderBottom:"1px solid #e8e8e8",padding:"0 20px",display:"flex",alignItems:"stretch"}}>
          ...
          {tab==="suporte"&&selSup&&(
            <div style={{marginLeft:"auto",display:"flex",alignItems:"center",padding:"0 8px",gap:10,fontSize:10,color:"#bbb"}}>
              <span><kbd style={{background:"#f5f5f5",border:"1px solid #e0e0e0",borderRadius:3,padding:"1px 5px",fontFamily:"monospace"}}>Esc</kbd> Fechar</span>
              <span><kbd style={{background:"#f5f5f5",border:"1px solid #e0e0e0",borderRadius:3,padding:"1px 5px",fontFamily:"monospace"}}>Enter</kbd> Iniciar</span>
              <span><kbd style={{background:"#f5f5f5",border:"1px solid #e0e0e0",borderRadius:3,padding:"1px 5px",fontFamily:"monospace"}}>R</kbd> Resolver</span>
            </div>
          )}
        </div>
      )}
      ...
      {showImp&&(
        <div style={{padding:32,maxWidth:620,margin:"0 auto"}}>
          ...
          <div style={{background:"#f8f8f8",borderRadius:4,border:"1px solid #e8e8e8",padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:"#999",marginBottom:10,fontWeight:500,letterSpacing:"0.08em",textTransform:"uppercase"}}>Mapeamento automático</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:11}}>
              {[["No NUVEM","Identificador Ecommerce"],["Destinatário","Destinatário Nome"],["Transportadora","Estratégia de Frete"],["Cód. Rastreio","Rastreador Last Mile"],["Status","Situação"],["Prazo","Prazo Logístico"]].map(([c,a])=>(
                <div key={c} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:3,padding:"7px 10px"}}>
                  <div style={{color:GOLD,fontWeight:500,marginBottom:2}}>{c}</div>
                  <div style={{color:"#aaa",fontSize:10}}>{a}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:10,marginTop:10}}>
            <button onClick={()=>doImport(paste)} disabled={!paste.trim()} style={{flex:1,background:"#000",border:"none",color:"#fff",borderRadius:3,padding:"11px 0",fontSize:12,fontWeight:500,cursor:paste.trim()?"pointer":"not-allowed",opacity:paste.trim()?1:0.4,letterSpacing:"0.08em",textTransform:"uppercase"}}>Importar dados colados</button>
      ...
      {!showImp&&tab==="dashboard"&&(
        <div style={{padding:"20px 24px"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
            ...
            <StatCard label="Resolvidos"    val={arch}     sub="arquivados"/>
          </div>
          {rows.length===0?(
            <div style={{textAlign:"center",padding:48,color:"#ccc",fontSize:13}}>Importe dados para visualizar os gráficos</div>
          ):(
            <div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16}}>
                <div style={{background:"#fff",borderRadius:6,border:"1px solid #e8e8e8",padding:16}}>
                  <div style={{fontSize:10,fontWeight:500,color:"#999",marginBottom:4,letterSpacing:"0.08em",textTransform:"uppercase"}}>Urgência</div>
                  <ResponsiveContainer width="100%" height={190}>
                    <PieChart><Pie data={urgData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={74} innerRadius={40} paddingAngle={3}>{urgData.map((e,i)=><Cell key={i} fill={PIE_COLORS[e.name]||GOLD}/>)}</Pie><Tooltip contentStyle={{fontSize:12}}/></PieChart>
                  </ResponsiveContainer>
                ...
                <div style={{background:"#fff",borderRadius:6,border:"1px solid #e8e8e8",padding:16}}>
                  <div style={{fontSize:10,fontWeight:500,color:"#999",marginBottom:4,letterSpacing:"0.08em",textTransform:"uppercase"}}>Status</div>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={statusData} layout="vertical" margin={{left:0,right:20,top:4,bottom:0}}><XAxis type="number" tick={{fontSize:10,fill:ct}} axisLine={false} tickLine={false}/><YAxis type="category" dataKey="name" width={120} tick={{fontSize:10,fill:ct}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{fontSize:12}}/><Bar dataKey="value" fill={GOLD} radius={[0,3,3,0]} name="Pedidos"/></BarChart>
                  </ResponsiveContainer>
                </div>
                <div style={{background:"#fff",borderRadius:6,border:"1px solid #e8e8e8",padding:16}}>
                  <div style={{fontSize:10,fontWeight:500,color:"#999",marginBottom:4,letterSpacing:"0.08em",textTransform:"uppercase"}}>SLA</div>
                  <ResponsiveContainer width="100%" height={190}>
                    <PieChart><Pie data={slaData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={74} innerRadius={40} paddingAngle={3}>{slaData.map((e,i)=><Cell key={i} fill={e.fill}/>)}</Pie><Tooltip contentStyle={{fontSize:12}}/></PieChart>
                  </ResponsiveContainer>
                  <div style={{display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap"}}>{slaData.map(e=><span key={e.name} style={{fontSize:10,color:ct,display:"flex",alignItems:"center",gap:4}}><span style={{width:7,height:7,borderRadius:"50%",background:e.fill,display:"inline-block"}}/>{e.name} ({e.value})</span>)}</div>
                </div>
              </div>
              {carrierData.length>0&&(
                <div style={{background:"#fff",borderRadius:6,border:"1px solid #e8e8e8",padding:16}}>
                  <div style={{fontSize:10,fontWeight:500,color:"#999",marginBottom:4,letterSpacing:"0.08em",textTransform:"uppercase"}}>Transportadoras</div>
                  <ResponsiveContainer width="100%" height={Math.max(100,carrierData.length*34)}>
                    <BarChart data={carrierData} layout="vertical" margin={{left:0,right:24,top:4,bottom:0}}><XAxis type="number" tick={{fontSize:10,fill:ct}} axisLine={false} tickLine={false}/><YAxis type="category" dataKey="name" width={130} tick={{fontSize:10,fill:ct}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{fontSize:12}}/><Bar dataKey="value" fill="#333" radius={[0,3,3,0]} name="Pedidos"/></BarChart>
                  </ResponsiveContainer>
                </div>
              )}
            </div>
        ...
        <div style={{padding:"16px 24px"}}>
          ...
          <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"55vh",borderRadius:4,border:"1px solid #e0e0e0"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:compact?11:12,tableLayout:"fixed",minWidth:1020}}>
              ...
              <thead>
                <tr>
                  <th style={thF}><input type="checkbox" onChange={e=>e.target.checked?setSelIds(new Set(pagedLog.map(r=>r.id))):clearSel()} checked={selIds.size>0&&pagedLog.every(r=>selIds.has(r.id))} style={{cursor:"pointer"}}/></th>
              ...
              <tbody>
                {pagedLog.length===0?(
                  <tr><td colSpan={13} style={{textAlign:"center",padding:28,color:"#bbb"}}>Nenhum pedido encontrado</td></tr>
                ):pagedLog.map((r,i)=>(
                  <tr key={r.id} style={{background:r.isNew?"rgba(196,150,58,0.12)":rowBg(r.urgencia,compact,i),borderBottom:"1px solid #f2f2f2",outline:r.isNew?`1px solid ${GOLD}`:undefined}}>
                    <td style={{padding:`${pd}px 6px`,textAlign:"center"}}><input type="checkbox" checked={selIds.has(r.id)} onChange={()=>toggleSel(r.id)} style={{cursor:"pointer"}}/></td>
                    ...
                    <td style={{padding:`${pd}px 10px`}}><button onClick={()=>upd(r.id,{enviadoSuporte:true,atendimento:"Aberto",sentAt:new Date().toISOString()},{acao:"Enviado ao suporte"})} style={{background:"transparent",border:"1px solid #ccc",color:"#333",borderRadius:2,padding:"3px 10px",fontSize:11,cursor:"pointer",width:"100%"}}>Enviar →</button></td>
                    <td style={{padding:`${pd}px 6px`,textAlign:"center"}}><button onClick={()=>del(r.id)} style={{background:"transparent",border:"none",color:"#ccc",cursor:"pointer",fontSize:15}}>×</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:10}}>
            ...
            {totalPages>1&&<div style={{display:"flex",gap:4,alignItems:"center"}}>
              <button onClick={()=>setLPage(n=>Math.max(1,n-1))} disabled={safeP===1} style={{...SEL,padding:"4px 10px",cursor:safeP===1?"not-allowed":"pointer",opacity:safeP===1?0.4:1}}>‹</button>
              <span style={{fontSize:12,color:"#bbb",padding:"0 8px"}}>{safeP} / {totalPages}</span>
              <button onClick={()=>setLPage(n=>Math.min(totalPages,n+1))} disabled={safeP===totalPages} style={{...SEL,padding:"4px 10px",cursor:safeP===totalPages?"not-allowed":"pointer",opacity:safeP===totalPages?0.4:1}}>›</button>
            </div>}
          </div>
        </div>
      )}
        ...
        <div style={{display:"flex",minHeight:520}}>
          <div style={{width:detail?"37%":"100%",borderRight:detail?"1px solid #ebebeb":"none",display:"flex",flexDirection:"column",background:"#fff"}}>
            ...
            <div style={{overflowY:"auto",flex:1,maxHeight:"64vh"}}>
              {ss.total===0?(
                <div style={{textAlign:"center",padding:"52px 20px",color:"#ccc"}}><div style={{fontSize:26,marginBottom:8,opacity:0.4}}>◎</div><div style={{fontSize:14,marginBottom:4}}>Fila vazia</div><div style={{fontSize:12}}>Pedidos enviados da Logística aparecem aqui</div></div>
              ):supRows.length===0?(<div style={{textAlign:"center",padding:24,color:"#ccc",fontSize:13}}>Nenhum resultado</div>)
              :supRows.map(r=>{
                const isSel=selSup===r.id
                const acColor=r.urgencia==="Alta"?"#e74c3c":r.urgencia==="Média"?GOLD:"#27ae60"
                return(
                  <div key={r.id} onClick={()=>setSelSup(isSel?null:r.id)} style={{padding:"11px 16px 11px 12px",cursor:"pointer",borderBottom:"1px solid #f5f5f5",borderLeft:`3px solid ${acColor}`,background:isSel?"#fdf8f0":"#fff",display:"flex",alignItems:"flex-start",gap:8}}>
                    <input type="checkbox" checked={selSupIds.has(r.id)} onClick={e=>e.stopPropagation()} onChange={()=>toggleSelSup(r.id)} style={{marginTop:3,cursor:"pointer",flexShrink:0}}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:3}}>
                        <span style={{fontWeight:500,fontSize:12,color:"#000"}}>{r.nuvem}</span>
          ...
          {detail?(
            <div style={{flex:1,display:"flex",flexDirection:"column",overflowY:"auto",maxHeight:"73vh",background:"#fafafa"}}>
              <div style={{background:"#fff",padding:"18px 24px 14px",borderBottom:"1px solid #e8e8e8",position:"sticky",top:0,zIndex:5}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
                  <div>
                    <div style={{fontSize:10,color:"#aaa",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Pedido em atendimento</div>
                    <div style={{fontSize:18,fontWeight:400,color:"#000",marginBottom:3}}>{detail.destinatario}</div>
                    <div style={{fontSize:12,color:"#888"}}>#{detail.nuvem} · NF {detail.nf} · {detail.transportadora}</div>
                  </div>
              ...
              <div style={{padding:"20px 24px",flex:1}}>
                ...
                <div style={{marginBottom:20}}>
                  <div style={{fontSize:9,color:"#aaa",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>Observações</div>
                  <textarea value={detail.obs||""} onChange={e=>upd(detail.id,{obs:e.target.value})} placeholder="Anotações do atendimento..." rows={3}
                    style={{width:"100%",borderRadius:3,border:"1px solid #d5d5d5",padding:"8px 10px",fontSize:12,resize:"vertical",fontFamily:"inherit",background:"#fff",color:"#000",boxSizing:"border-box"}}/>
                </div>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                        {[{ch:"wpp",label:"WhatsApp"},{ch:"zendesk",label:"Zendesk"}].map(({ch,label})=>(
                          <div key={ch} style={{background:"#fff",borderRadius:3,border:"1px solid #e0e0e0",overflow:"hidden"}}>
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",borderBottom:"1px solid #e0e0e0",background:"#000"}}>
                              <span style={{fontSize:11,color:GOLD,fontWeight:500,letterSpacing:"0.06em"}}>{label}</span>
                ...
                <div style={{border:"1px solid #e8e8e8",borderRadius:4,overflow:"hidden",background:"#fff"}}>
                  ...
                  {openHist&&(
                    <div style={{padding:14,borderTop:"1px solid #f0f0f0",background:"#fafafa"}}>
                      {detail.historico.length===0?<div style={{fontSize:12,color:"#bbb"}}>Nenhuma ação registrada.</div>:(
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          {detail.historico.map((h,i)=>(
                            <div key={i} style={{display:"flex",gap:10,alignItems:"center",fontSize:12}}>
                              <span style={{color:"#bbb",whiteSpace:"nowrap",fontSize:11,minWidth:130}}>{h.ts}</span>
                              <span style={{width:5,height:5,borderRadius:"50%",background:GOLD,flexShrink:0}}/>
                              <span style={{color:"#333"}}>{h.acao}</span>
                            </div>
                          ))}
                        </div>
                      )}
            ...
            <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",background:"#fafafa"}}>
              <div style={{textAlign:"center"}}><div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:"#ccc",marginBottom:8}}>Selecione um pedido para atender</div><div style={{fontSize:10,color:"#ddd"}}>ou selecione vários para arquivar em lote</div></div>
            </div>
          ):null}
        </div>
      )}
      ...
      {!showImp&&tab==="arquivados"&&(
        <div style={{padding:"16px 24px"}}>
          ...
          {arch===0?(
            <div style={{textAlign:"center",padding:"44px 0",color:"#ccc"}}><div style={{fontSize:28,marginBottom:10,opacity:0.3}}>◎</div><div style={{fontSize:14,marginBottom:6}}>Nenhum atendimento arquivado</div><div style={{fontSize:12}}>Pedidos resolvidos no Suporte aparecem aqui</div></div>
          ):(
            <div>
                  ...
                  <tbody>
                    ...
                    :archRows.map((r,i)=>(
                      <tr key={r.id} style={{background:i%2===0?"#fff":"#fafafa",borderBottom:"1px solid #f2f2f2"}}>
                        ...
                        <td style={{padding:`${pd}px 10px`}}><button onClick={()=>upd(r.id,{atendimento:"Em andamento"},{acao:"Reaberto dos arquivados"})} style={{background:"transparent",border:"1px solid #ccc",color:"#333",borderRadius:2,padding:"3px 10px",fontSize:11,cursor:"pointer"}}>Reabrir</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
